...
{  //滑动平均滤波 - 与总的通道数匹配
  static u32  ADC_Read_Cnt = 0;                   //轮询AD7799转换结果的时间间隔计数器 
...
/*******************************************************************************
* Function Name  : ad_update
* Description    : 清除7799缓存
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
void ad_update(void)
{
  ...
  for(i = 0; i < AD7799_CHANN_USE; i++)
  {
    ...
    MovingAv_Fill[i] = 0;  
    MovingAv_Cnt[i] = 0;    
    for(j=0;j<MOVING_AVERAGE_BUFSIZE;j++) MovingAv_Buf[i][j]=0;

  //中位值滤波 - 与芯片数匹配
  }
  MedianFilt_Cnt[0] = 0;    //中位值滤波 - 数据计数器
  for(i=0;i<MEDIAN_FILTERING_BUFSIZE;i++)
  MedianFilt_Buf[0][i]=0;   //中位值滤波 - 数据缓存
}