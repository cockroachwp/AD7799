...
V2.1:修改串口中断服务程序，增加中断标志判断及溢出清零
V2.2:增加总线异常判断，超过5S主控未与模块通信，总线异常标志置1，模块自动进入安全模式
*****增加IAP功能所需代码
*****增加通信LED闪烁功能
********************************************************************************/
/* Includes ------------------------------------------------------------------*/
...
u8  BUS_ERR = 0;               //总线异常，超过5S总线上没有数据，该位置1
...
void RS485_Init(void)
{
  ...
  /************************  通信指示灯端口配置   ******************************/    
#ifdef RS485_LED_EN           //通信指示灯闪烁使能
  GPIO_InitStructure.GPIO_Pin = RS485_LED_PIN;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_Init(RS485_LED_GPIO, &GPIO_InitStructure); 
#endif    
  /************************  RS485端口及串口配置   ******************************/  
  /* 配置485通信TX端口 */
  ...
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_Init(GPIO_RD, &GPIO_InitStructure);
  ...
  TIM_PrescalerConfig(RS485_TIM, 7199, TIM_PSCReloadMode_Immediate);

  /* Output Compare Active Mode configuration: Channel1-通信断帧*/
  TIM_OCInitStructure.TIM_OCMode = TIM_OCMode_Inactive;
  TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Enable;
  TIM_OCInitStructure.TIM_Pulse = 5;           //定时6ms
  TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_High;
  
  TIM_OC1Init(RS485_TIM, &TIM_OCInitStructure);
  TIM_OC1PreloadConfig(RS485_TIM, TIM_OCPreload_Disable);
  
  /* Output Compare Active Mode configuration: Channel2-通信总线异常定时*/
  TIM_OCInitStructure.TIM_OCMode = TIM_OCMode_Inactive;
  TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Enable;
  TIM_OCInitStructure.TIM_Pulse = 50000;           //定时5s
  TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_High;
  
  TIM_OC2Init(RS485_TIM, &TIM_OCInitStructure);
  TIM_OC2PreloadConfig(RS485_TIM, TIM_OCPreload_Disable);
  TIM_ClearITPendingBit(RS485_TIM, TIM_IT_CC2); //定时中断标志清零
  TIM_ITConfig(RS485_TIM, TIM_IT_CC2, ENABLE);  //中断使能
...
void BUS_DataResponse(void)
{
  ...
  {    
#ifdef RS485_LED_EN           //通信指示灯闪烁使能
    RS485_LED_STATE?RS485_LED_ON:RS485_LED_OFF;
#endif     
    ...
    case BUS_WR8:                      //向丛机写数据0
      BUS_Buffer.WR8[0] = 1;           //表示有新的数据更新
      for(i=2;i<RxBufferLength-2;i++)  //去除地址，功能和CRC码
      {
        BUS_Buffer.WR8[i-1] = RxBuffer[i];
      }
      TxBuffer[0] = LocalAddress;      //本机地址
      TxBuffer[1] = BUS_WR8;           //功能码
      CRC_CODE = CRC16(TxBuffer,2);    //计算CRC码
      TxBuffer[2] = (CRC_CODE & 0x00FF);        //CRC校验码的低字节
      TxBuffer[3] = ((CRC_CODE >> 8) & 0x00FF); //CRC校验码的高字节
      RS485_SendData(MIN_BufferSize);           //向上位机发送响应数据      
      /*******************软件复位命令****************************/     
#ifdef IAP_RESET_EN           //IAP软件复位使能
      __set_FAULTMASK(1);     //关闭所有异常及中断响应
...
void TIM_IRQ(void)
{
  ...
  else if(TIM_GetITStatus(RS485_TIM, TIM_IT_CC2) != RESET)
  {
    TIM_ClearITPendingBit(RS485_TIM, TIM_IT_CC2); //定时中断标志清零
    BUS_ERR = 1;
  }
